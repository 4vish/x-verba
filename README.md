...
A free CLI that scans any codebase, finds unprotected decision points, maps agent and decision graphs, and generates a governance template you can actually use. Works on any repo. No account required.
Built by Super Semantics.
Requirements: Python 3.10+ or Node.js 18+. Supports `.py`, `.js`, `.ts`, `.jsx`, `.tsx`, `.java`, `.go`, `.rb`, `.cs`. Schema version 1.0.
---
What's new in v0.4.0
Governance Intelligence — overall and critical governance coverage percentages, broken down by decision type and by consequence type
Tendency Analysis — a five-state classification (`stable` / `emerging` / `amplifying` / `critical` / `failure`) describing where your system is heading if left ungoverned, plus T-Amplification and Pre-Node Proximity indicators
Gamma Variants — Structural Gamma broken down by overall, critical, agent-handover, agent-chain, and cluster scope, each with an ABOVE_THRESHOLD / PARTIAL_COVERAGE / BELOW_THRESHOLD status
Agent Graph — agent inventory, handover detection (governed vs. ungoverned), chain governance, and cluster risk scoring
Decision Graph — every decision point as a graph node, with PageRank-based influence ranking, critical-path analysis, and propagation-potential analysis (which downstream decisions a given decision can reach)
9-section governance scorecard via `x-verba scan --format text`
---
Install
```bash
...
Structural Gamma:       0.1364  (BELOW_THRESHOLD)
Tendency:               FAILURE

CRITICAL: 16  HIGH: 12  MEDIUM: 1

Top influential decisions:
  >  sample.ts:47  (PageRank: 0.019122)
  >  sample.rs:26  (PageRank: 0.015000)
  >  sample.ts:40  (PageRank: 0.011668)

Report: .verba/governance-report.txt
```
The full 9-section governance scorecard — Governance Scorecard, AI/Agent/Decision Visibility, Governance Coverage, Critical Decisions, Governance Gaps Summary, Drift Class Detections, and Governance Recommendations — is written to the report file. See "Example output" below.
---
Features at a glance
Scans Python (AST-based), JavaScript/TypeScript, Go, Rust, and C# repos for governance gaps
...
Computes a transparent Structural Gamma score (Γ) and Gamma Variants (overall, critical, agent-handover, agent-chain, cluster)
Computes Governance Coverage (overall, critical, by decision type, by consequence type)
Classifies system Tendency (stable / emerging / amplifying / critical / failure) with T-Amplification and Pre-Node Proximity indicators
Builds an Agent Graph (handovers, chains, clusters) and a Decision Graph (PageRank, critical path, propagation potential)
9-section governance scorecard (`x-verba scan --format text|json`)
...
---
What X-Verba does — and does not do
X-Verba does
Scan code statically for structural governance gaps — missing Pre-Nodes, ungated irreversible actions, unvalidated AI inputs, ungoverned agent handovers
Propose Candidate Governance Nodes — inferences about the consequential behavioural states your system reaches, derived from observable implementation transitions
Detect Drift Class candidates — proposed failure mode mappings based on structural evidence in the code
Generate a governance contract scaffold with every field explained in plain English, leaving policy fields blank for your team to complete
Compute Structural Gamma (governance coverage proxy), Gamma Variants, Tendency, and Governance Coverage
Build Agent and Decision Graphs — who calls whom, what is governed, what is not

X-Verba does not
Infer your governance policy. Only you know what your system must and must never do. The scanner proposes; your engineering team decides.
Enforce governance at runtime. Static analysis finds structural gaps. Enforcement at runtime requires the VERBA Priming Engine, which is a separate system.
Replace human review. Every governance contract the scanner generates is a scaffold — a structured starting point, not a finished policy. The null fields are intentional.
Guarantee compliance. X-Verba generates structured artifacts that support a compliance case. It does not validate that your governance policy is correct, complete, or sufficient for your regulatory context.
Confirm Candidate Nodes. The scanner proposes behavioural state names based on code patterns. A human engineer confirms or corrects every candidate_node and fills in confirmed_node_name before the contract is approved.

Why human confirmation is required
The scanner can observe implementation transitions — where an AI call happens, what method is invoked, where the output goes. It cannot observe intent. It cannot know whether EMBEDDINGS_GENERATED in your system means a routine background task or a decision that determines a patient's treatment. Only you know that. Governance policy is the answer to questions the code cannot answer. X-Verba asks the right questions and generates the workspace for your team to answer them.
---
The Gamma score (Γ)
...
---
Candidate Governance Node
A Candidate Governance Node is X-Verba's inference about the consequential behavioural state an implementation transition produces. The scanner analyses observable transitions — AI calls, API calls, agent handovers, database commits — and infers the most likely resulting state. That inference is a candidate. A human engineer confirms or corrects it.
The distinction that matters:
```
execute_payment()       ← implementation transition (the code X-Verba observes)
PAYMENT_EXECUTED        ← Candidate Governance Node (the behavioural state X-Verba infers)
```
Governance applies to the state, not the implementation. The implementation may be refactored, replaced, or abstracted. The governance requirement — that PAYMENT_EXECUTED must have a Pre-Node, an audit trail, and a Terminal State on failure — does not change when the code does. This is why VERBA governs Nodes, not function calls.
In the governance contract: the `candidate_node` field is the scanner's inference. The `inference_confidence` field is HIGH, MEDIUM, or LOW. The `confirmed_node_name` field is blank — it is for your team to fill in. A contract with an unconfirmed `candidate_node` is a proposal. A contract with a confirmed `confirmed_node_name` is a governance commitment.
---
Cluster
...
---
Legion
A Legion is a typed drift pattern hypothesis — a specific, named code signature that suggests a Drift Class may be present or forming. Each Drift Class contains one or more Legions, each representing a distinct observable form of that failure mode.
Legions are evidence, not verdicts. A HIGH-confidence Legion means the structural signature is present in the code. Whether that signature represents an actual governance risk in your specific system is for your team to determine.
Confidence tiers:
HIGH (0.9) — pattern confirmed directly from call graph or control-flow analysis. Treat as a confirmed structural finding pending human review.
MEDIUM (0.6) — specific structural pattern, likely correct, worth investigating.
SPECULATIVE (0.3) — keyword match only, no verified structural signature. Flag for human review before acting on.
Example: DC-E13 (Ungated Agent Handover) contains three Legions:
`E13-L1 Ungated Handover` — an agent handover with no Pre-Node detected. HIGH confidence, structural.
`E13-L2 Cluster Governance Gap` — a cluster of agents with no cluster-level governance. HIGH confidence, structural.
`E13-L3 Sourceless Cascade` — multiple chained AI calls with no traceable origin validation. MEDIUM confidence, structural.
Legions appear in the governance contract under Section 9b (Drift Class Detection). Each Legion includes the matched location, confidence tier, evidence, and a recommended Stabilisation Operator.
---
...
Five modes
`x-verba scan`
Scan any repo and generate a 9-section governance scorecard.
```bash
x-verba scan ./my-repo                              # text scorecard (default)
x-verba scan ./my-repo --format json                # full machine-readable results
x-verba scan ./my-repo --identity-key my-system-v1.0
x-verba scan ./my-repo --context-profile ai-app     # ai-app | system-utility | general
x-verba scan ./my-repo --strict-ai-only             # exit non-zero if no AI integrations found
```
`--format text` (default) writes the full governance scorecard to `.verba/governance-report.txt`: Governance Scorecard (Structural Gamma + Tendency State), AI Visibility, Agent Visibility, Decision Visibility, Governance Coverage, Critical Decisions, Governance Gaps Summary, Drift Class Detections, and Governance Recommendations. A condensed summary also prints to the terminal. See "Example output" below.
`--format json` writes the same scan results as a single machine-readable object (`summary`, `graphs`, `inventories`, `metrics`, `algorithms`, `gaps`, `drift_classes`) to `.verba/governance-report.json`, for piping into other tools or CI dashboards.
The fillable governance contract (`governance.yaml`, sections 1-14 — including the new Governance Intelligence, Agent Governance, and Critical Decisions sections) is produced from the same scan results by the output writer. Wiring this up as a `scan --format yaml` flag is tracked for v0.5 — see Roadmap.
---
Example output — governance scorecard (`--format text`, excerpt)
```
════════════════════════════════════════════════════════════════════════════════
X-VERBA SCAN RESULTS
════════════════════════════════════════════════════════════════════════════════

Repository:  ./my-repo
Scan Date:   2026-06-14T09:30:02.488659+00:00

════════════════════════════════════════════════════════════════════════════════
GOVERNANCE SCORECARD
════════════════════════════════════════════════════════════════════════════════

Structural Gamma:                  0.14 / 1.0  [BELOW_THRESHOLD]
  ├─ Total Decision Points:        44
  ├─ Governed Decision Points:     6 (14%)
  └─ Ungoverned Decision Points:   38 (86%)

Tendency State:                     FAILURE
  ├─ T-Amplification Active:       No (10 high-centrality ungoverned decisions)
  └─ Pre-Node Proximity:           CRITICAL (>50% of critical decisions ungoverned)

════════════════════════════════════════════════════════════════════════════════
AI VISIBILITY
════════════════════════════════════════════════════════════════════════════════

AI Integrations Detected:          13
  ├─ Governed:                     1 (8%)
  ├─ Ungoverned:                   12 (92%)
  └─ High-Risk Patterns:           0 (high temperature + user input + dynamic prompt)
...
Agents Detected:                    2
Agent Handovers Detected:           1
  ├─ Governed:                     1 (100%)
  └─ Ungoverned:                   0 (0%)

Agent Chains:                        1
  ├─ Fully Governed:               1
  ├─ Partially Governed:           0
  └─ Ungoverned:                   0
...
Decision Points Detected:            44
  ├─ Low Consequence          0 (0%)
  ├─ Medium Consequence       0 (0%)
  ├─ High Consequence         41 (93%)
  └─ Critical Consequence     3 (7%)

Decision Influence Ranking (PageRank):
  ├─ Most Influential Decision:   sample.ts:47 (score=0.0191, criticality=0.50)
  └─ Governance Choke Point:      sample.ts:47 (score=0.0191, ungoverned)
...
... (Critical Decisions, Governance Gaps Summary, Drift Class Detections,
     and Governance Recommendations sections follow)
```
---
`x-verba qa`
...
Honest status
Working today (v0.4):
Scans Python (AST-based), JavaScript/TypeScript, Go, Rust, and C# repos. Detects AI API calls, ungated irreversible actions, missing Pre-Nodes, informal Invariants, agent handovers, and cluster-level governance risks. Covers all 36 DC classes in the classification taxonomy. Builds an Agent Graph (handovers, chains, clusters) and a Decision Graph (PageRank influence ranking, critical path, propagation potential). Computes Structural Gamma, Gamma Variants (overall, critical, agent-handover, agent-chain, cluster), Governance Coverage (overall, critical, by decision type, by consequence type), and a five-state Tendency classification. `x-verba scan --format text|json` produces the full 9-section governance scorecard.
Known limitations:
JS/TS, Go, Rust, and C# scanners use pattern-based detection and will miss some custom wrappers. The Python AST scanner will miss dynamically constructed calls. Gamma and its variants remain structural proxies, not runtime Gamma. The YAML/JSON/Markdown governance contract (sections 1-14, including the new Governance Intelligence, Agent Governance, and Critical Decisions sections) is implemented in the output writer but not yet exposed as a `scan` CLI flag.
In active development (v0.5):
`scan --format yaml` wired to the governance-contract writer. `x-verba qa` regression checks with CI/CD exit codes and a `--strict` Gamma threshold. `x-verba forensics` log and trace ingestion.
v0.6 and beyond:
Governed prompt generation. Governance diff viewer. Drift heatmaps. IDE integration.
...
Roadmap
v0.1-v0.3 — Done
`x-verba scan` — Python (AST), JavaScript/TypeScript, Go, Rust, C#
Full 36-class runtime detection, Tier A-D taxonomy in reports
YAML, JSON, Markdown governance template (output writer)
Structural Gamma score
v0.4 — Now
Governance Intelligence — Governance Coverage (overall/critical, by decision type, by consequence type)
Tendency Analysis — five-state classification, T-Amplification, Pre-Node Proximity
Gamma Variants — overall, critical, agent-handover, agent-chain, cluster
Agent Graph — agent inventory, handover/chain/cluster governance
Decision Graph — PageRank influence ranking, critical path, propagation potential
9-section governance scorecard (`x-verba scan --format text|json`)
v0.5
`scan --format yaml` wired to the governance-contract writer (sections 1-14)
`x-verba qa` — CI/CD integration, exit codes, `--strict` Gamma threshold
